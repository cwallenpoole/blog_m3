...
class Fill(Handle):
    ...
    def h_line(self, i):
        ...
        for x in i[THING]:
            if isinstance(x, str):
                ...
                raise WTF(x)
        return ''.join(list(map(lambda x: x.decode('utf-8') if isinstance(x,bytes) else str(x), out)))
    ...
    def h_lines(self, lines):
        ...
        return list(map(self.h, lines))
    
    def go(self):
        self.output = Stowage()
        self.output._str = ''.join(map(self.h, self.parsetree))
        if list(self.output.keys()) == ['_str']:
