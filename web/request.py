...
import sys, re, types, os.path, urllib.request, urllib.parse, urllib.error

from . import http, wsgi, utils, webapi
    ...
    for url, ofno in utils.group(mapping, 2):
        ...
        if result: # it's a match
            ...
            for d in re.findall(r'\\(\d+)', ofn):
                args.pop(int(d) - 1)
            return tocall(*([urllib.parse.unquote(x) for x in args] + fna))
...
def run(inp, fvars, *middleware):
    ...
    **Caveat:** So that `reloader` will work correctly, input has to be a variable,
    ...
    r = wsgi.runwsgi(webapi.wsgifunc(webpyfunc(inp, fvars, autoreload), *middleware))
    return r
