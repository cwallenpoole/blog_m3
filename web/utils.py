...
def group(seq, size): 
    ...
    if not hasattr(seq, 'next'):  
        seq = iter(seq)
    while True: 
        yield [next(seq) for i in range(size)]
...
class ThreadedDict:
    ...
    def __getattr__(self, attr): 
        ...
        if th in self.__d:
            return getattr(self.__d[th], attr, None)
        else:
            self.__d[th] = Storage()
        return None
    ...
    def __setattr__(self, attr, value):
        if attr == '__doc__':
            ...
            th = threading.currentThread()
            if not th in self.__d:
                self.__d[th] = Storage()
            return setattr(self.__d[th], attr, value)
