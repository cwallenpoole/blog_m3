    ...
    def db_execute(cur, sql_query):
        ...
        try:
            ...
            if web.config.get('db_printing'):
                print('ERR:', str(sql_query), file=web.debug)
            rollback()
            raise

        if web.config.get('db_printing'):
            print('%s (%s): %s' % (round(b-a, 2), web.ctx.dbq_count, str(sql_query)), file=web.debug)
...
def sqlwhere(dictionary, grouping=' AND '):
    ...
    return SQLQuery(grouping.join([
      '%s = %s' % (k, aparam()) for k in list(dictionary.keys())
